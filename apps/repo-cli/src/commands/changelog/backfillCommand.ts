...
export const changelogBackfillCommand = createCommandDefinition({
    name: 'changelog:backfill',
    description: 'Backfill .changelogs JSON from first commit to first tag.',
    argsSchema: z.object({
    ...
    optionsSchema: z.object({
        all: z.boolean().meta({ description: 'Backfill all packages.' }).optional(),
    ...
    action: async ({ container, args: { packagePath }, options }) => {
        ...
        outputService.logType({
            type: 'info',
            message: options.dry ? 'Backfill report (preview)' : 'Backfill report',
            obj: report,
        });
